...
export const fetchProducts = async (page = 1, limit = 20, search = '', maxPrice?: number) => {
  ...
  let products = (Array.isArray(dbData?.products) ? dbData.products : []).map((p: any) => ({
    id: p?.id,
    name: p?.name || 'Product',
    price: Number(p?.price || 0),
    image: String(p?.image || ''),
    neworold: typeof p?.neworold === 'boolean' ? p.neworold : null,
    isRealBrand: typeof p?.isRealBrand === 'boolean' ? p.isRealBrand : null,
  ...
  return {
    products,
    total: typeof dbData?.total === 'number' ? dbData.total : products.length,
    hasMore: typeof dbData?.totalPages === 'number' ? page < dbData.totalPages : products.length >= limit
  };
}
...
export async function fetchProductById(id: number | string) {
  ...
  const normalizedId = rawId.replace(/^rapid-/i, '').trim();
  const data = await request(`/products/${encodeURIComponent(normalizedId)}`, { skipCache: true });
...
export async function searchProducts(query: string, page = 1, limit = 20) {
  ...
  return {
    products,
    total: typeof rapidData?.total === 'number' ? rapidData.total : products.length,
    hasMore: Boolean(rapidData?.hasMore)
  };
}

export async function fetchSearchSuggestions(query: string, limit = 8) {
  const encodedQuery = encodeURIComponent(query || '');
  return request(`/search/suggestions?q=${encodedQuery}&limit=${limit}`);
}

export async function convertItemIdStr(itemIdStr: string) {
  return request('/tools/itemidstr-convert', {
    method: 'POST',
    body: JSON.stringify({ itemIdStr })
  });
}
...
export async function rapidSearchItemsByImage(_imgUrl: string, page = 1, pageSize = 20) {
  const dbData = await fetchProducts(page, pageSize, '');
  return {
    items: Array.isArray(dbData?.products) ? dbData.products : [],
    total: typeof dbData?.total === 'number' ? dbData.total : 0,
    hasMore: Boolean(dbData?.hasMore)
  };
}

export async function rapidGetItemDetail(itemId?: string, itemIdStr?: string) {
  const fallbackId = String(itemId || itemIdStr || '').trim();
  return fetchProductById(fallbackId);
}
...
export async function fetchProductReviews(productId: number | string) {
  ...
  const rawId = String(productId || '').trim().replace(/^rapid-/i, '');
  const dbReviews = await request(`/products/${encodeURIComponent(rawId)}/reviews`, { skipCache: true }).catch(() => null);
  if (Array.isArray(dbReviews)) return dbReviews;
  if (Array.isArray((dbReviews as any)?.reviews)) return (dbReviews as any).reviews;
  return [];
}
