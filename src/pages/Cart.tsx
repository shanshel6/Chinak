...
import { AlertCircle, ShoppingCart, ArrowLeft, RefreshCw, Heart, Trash2, Tag, X, ArrowRight, CheckCheck, Truck, Minus, Plus } from 'lucide-react';
  ...
  if (error) {
    return (
      <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark items-center justify-center p-4 text-text-primary-light dark:text-text-primary-dark font-display antialiased pt-safe" dir="rtl">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-center mb-8">{error}</p>
        <button 
          onClick={() => fetchCart()}
                ...
                <button 
                  ...
                  className={`size-11 rounded-2xl shadow-sm flex items-center justify-center transition-all border ${
                    selectedItems.length > 0 
                      ...
                      : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-white/5 opacity-50'
                  }`}
                >
                  <Trash2 size={20} />
                </button>
      ...
      <div className="sticky top-[calc(env(safe-area-inset-top)+3.5rem)] z-40 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-xl pt-2 pb-2 px-4 shadow-sm border-b border-slate-100/50 dark:border-slate-800/50">
        <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-[1.2rem] relative isolate">
          ...
          >
            ...
            {seaCount > 0 && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full transition-colors ${
                activeTab === 'sea' ? 'bg-primary/10 text-primary' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
              }`}>
                {seaCount}
              </span>
            )}
                ...
                {!isSelectMode && (
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <Heart size={20} className={isProductInWishlist(item.product.id) ? 'fill-red-500 text-red-500' : 'text-slate-400 dark:text-slate-500'} />
                      </button>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-700/50 rounded-xl p-1">
                      <button
                        ...
                        className="w-9 h-9 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed active:scale-95 transition-all"
                        aria-label="إنقاص الكمية"
                      >
                        <Minus size={16} />
                      </button>
                      <span className="min-w-[2ch] text-center text-sm font-black text-slate-800 dark:text-slate-100 tabular-nums" aria-live="polite">
                        {item.quantity}
                      </span>
                      <button
                        onClick={(e) => {
                        ...
                        className="w-9 h-9 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 shadow-sm active:scale-95 transition-all"
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-4">
            <div className="flex flex-col">
                ...
                <span className="text-xl font-black text-slate-900 dark:text-white leading-tight">
                    ...
                    <span className="text-xs font-bold text-slate-400 dark:text-slate-500 mr-1">د.ع</span>
                </span>
            </div>
