...
const PDD_SKIP_COOKIES = process.env.PDD_SKIP_COOKIES === '1';
const PDD_IDLE_AFTER_LOGIN_MS = Number(process.env.PDD_IDLE_AFTER_LOGIN_MS || 15000);
...
const EDGE_OVERRIDE = process.env.PDD_EDGE_PATH || '';
const EDGE_PATHS = [
...
function getExecutablePath() {
  if (EDGE_OVERRIDE && fs.existsSync(EDGE_OVERRIDE)) return EDGE_OVERRIDE;
...
async function humanizePage(page, mode = 'light') {
    if (!page) return;
    const viewport = page.viewport() || { width: 360, height: 800 };
    const ranges = {
        light: { moves: [2, 4], scrolls: [0, 1] },
        medium: { moves: [3, 6], scrolls: [1, 2] },
        heavy: { moves: [5, 9], scrolls: [2, 3] }
    };
    const cfg = ranges[mode] || ranges.light;
    const moveCount = Math.floor(cfg.moves[0] + Math.random() * (cfg.moves[1] - cfg.moves[0] + 1));
    const scrollCount = Math.floor(cfg.scrolls[0] + Math.random() * (cfg.scrolls[1] - cfg.scrolls[0] + 1));
    for (let i = 0; i < moveCount; i++) {
        const x = Math.max(5, Math.floor(Math.random() * (viewport.width - 10)));
        const y = Math.max(5, Math.floor(Math.random() * (viewport.height - 10)));
        try {
            await page.mouse.move(x, y, { steps: 8 + Math.floor(Math.random() * 12) });
        } catch (e) {}
        await delay(150 + Math.floor(Math.random() * 350));
    }
    for (let i = 0; i < scrollCount; i++) {
        const delta = (Math.random() > 0.5 ? 1 : -1) * (80 + Math.floor(Math.random() * 220));
        try {
...
async function createBrowser() {
  ...
  const useFreshProfile = String(process.env.PDD_PROFILE_FRESH || '') === '1';
  const userDataDir = useFreshProfile
    ? `chrome_data_pdd_${Date.now()}`
    : 'chrome_data_pdd_persistent';
...
async function run() {
    ...
    const MOBILE_UA = 'Mozilla/5.0 (Linux; Android 12; Pixel 5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Mobile Safari/537.36';
    try {
    ...
    try {
        await safeGoto(page, 'https://mobile.pinduoduo.com/', { waitUntil: 'domcontentloaded', timeout: PAGE_LOAD_TIMEOUT });
        await delay(4000);
        await humanizePage(page, 'medium');
        await humanDelay(6000, 12000);
        await humanizePage(page, 'heavy');
        await humanDelay(4000, 8000);
    } catch (e) {}
    await humanDelay(2500, 4500);
    await humanizePage(page, 'light');
    ...
    await delay(30000); 
    if (PDD_IDLE_AFTER_LOGIN_MS > 0) {
        await delay(PDD_IDLE_AFTER_LOGIN_MS);
        await humanizePage(page, 'medium');
    }
    await page.setViewport({ width: 360, height: 800, isMobile: true, hasTouch: true });
    try {
        ...
        for (const point of clickPoints) {
            ...
            try {
                ...
                try {
                    await newPage.waitForLoadState ? newPage.waitForLoadState('domcontentloaded') : newPage.waitForSelector('body', { timeout: 15000 });
                    await humanDelay(2000, 4000);
                    await humanizePage(newPage, 'medium');
                } catch (e) {
