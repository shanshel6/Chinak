...
// Quotation API
export const fetchQuotations = async (page = 1, limit = 20) => {
  const response = await api.get(`/api/admin/quotations?page=${page}&limit=${limit}`);
  return response.data;
};

export const fetchQuotation = async (id: number) => {
  const response = await api.get(`/api/admin/quotations/${id}`);
  return response.data;
};
...
export const updateQuotation = async (id: number, data: any) => {
  const response = await api.put(`/api/admin/quotations/${id}`, data);
  return response.data;
};
