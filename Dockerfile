...
# Start the server
CMD ["sh", "-c", "npm install --omit=dev --omit=optional --no-audit --no-fund --ignore-scripts --legacy-peer-deps --verbose && npm install --os=linux --cpu=x64 sharp && npx prisma generate && node index.js"]
