...
// NOTE: CLIP text/image embedding generation has been removed from the Railway
// backend. All embeddings are generated on-device (client app) or by the local
// scraping pipeline. The backend only performs vector search on supplied vectors.
...
// Semantic (BGE-M3) hybrid search is DISABLED: the Railway backend no longer
// generates query embeddings. Search falls back to the keyword/DB engines.
const ENABLE_SEMANTIC_SEARCH = false;
...
const enqueueImageEmbeddingJob = (_productId) => {
  // DISABLED: CLIP image (vision) embedding no longer runs on the Railway
  // backend. Product image embeddings are generated by the local scraping
  // pipeline (on your PC) and written directly to the database.
  return;
};

const runImageEmbeddingJobs = async () => {
  // DISABLED: see enqueueImageEmbeddingJob. The Railway backend does not
  // generate CLIP image embeddings; the local pipeline handles that.
  return;
};
...
const expandSearchTermsForIraqiSlang = (query) => {
  ...
  const addCandidates = (value) => {
    ...
    if (typeof directMapped === 'string') {
      const cleanCandidate = String(directMapped || '').trim();
...
app.get('/api/tools/rapid/item-detail', async (req, res) => {
  ...
  app.get('/api/products/search-by-photo', async (req, res) => {
    ...
    try {
      // Server-side embedding has been removed. The on-device CLIP model
      // generates the text embedding; clients must POST that vector to
      // /api/search/embedding with { type: 'text' }.
      return res.status(410).json({
        error: 'Server-side text embedding has been removed. Generate the embedding on-device and POST it to /api/search/embedding with { type: "text" }.',
        engine: 'deprecated'
      });
    } catch (error) {
      console.error('[Photo Search] deprecated endpoint error:', error);
...
app.post('/api/search/image', async (req, res) => {
  try {
    // Server-side image embedding removed. Generate the CLIP image embedding
    // on-device and POST it to /api/search/embedding with { type: 'image' }.
    return res.status(410).json({
      error: 'Server-side image embedding has been removed. Generate the embedding on-device and POST it to /api/search/embedding with { type: "image" }.',
      engine: 'deprecated'
    });
  } catch (error) {
    console.error('[CLIP image search] deprecated endpoint error:', error);
...
app.post('/api/search/image-crop', upload.single('image'), async (req, res) => {
  try {
    // Server-side image embedding removed. Cropping + embedding happen on-device;
    // POST the resulting vector to /api/search/embedding with { type: 'image' }.
    return res.status(410).json({
      error: 'Server-side image-crop embedding has been removed. Crop and embed on-device, then POST the vector to /api/search/embedding with { type: "image" }.',
      engine: 'deprecated'
    });
  } catch (error) {
    console.error('Search image crop deprecated endpoint error:', error);
...
app.get('/api/search/categories', async (req, res) => {
  try {
    ...
    // Server-side query embedding removed: category search uses keyword/alias
    // scoring only. No MiniLM query embedding runs on the Railway backend.
...
app.all('/api/search', async (req, res) => {
  ...
  try {
    ...
    // Use the provided vector if it's available (from on-device CLIP ViT-B/32)
    if (vectorParam) {
      try {
    ...
    // Server-side embedding removed: the client must supply a precomputed
    // on-device CLIP vector. The backend no longer translates + embeds queries.
    if (!textEmbedding) {
      return res.status(400).json({
        error: 'A precomputed 512-dim CLIP vector is required (param "vector"). Server-side embedding is disabled; generate it on-device.',
        engine: 'client-embedding-required'
      });
    }
