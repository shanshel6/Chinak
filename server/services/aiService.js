...
export async function processProductEmbedding(productId) {
  // DISABLED: server-side embedding generation has been removed. Product
  // embeddings are produced by the local scraping pipeline (on your PC) and
  // written directly to the database. This is now a no-op.
  console.log(`[AI Debug] processProductEmbedding is disabled (no server-side embedding). Skipping product ${productId}.`);
}
...
export async function processProductAI(productId) {
  try {
    ...
    // Embedding generation removed from the backend: product embeddings are
    // generated by the local scraping pipeline. We only persist AI metadata here.
    console.log(`[AI Debug] Saving AI metadata for product ${productId} (embedding generation disabled)...`);
    ...
    await prisma.$executeRawUnsafe(query, aiMetadataParam, productId);
    console.log(`[AI Debug] Successfully processed product ${productId} (metadata only)`);
  } catch (error) {
...
export function normalizeArabic(text) {
  ...
  // Only character normalization, no static synonyms (use AI only)
  const words = normalized.split(/\s+/);
  return {
    fullString: normalized,
    groups: [words]
  };
}
