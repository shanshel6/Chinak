...
function callSiliconFlowLLM(messages, { model, temperature = 0.3, maxTokens = 200, timeoutMs = 45000 } = {}) {
  ...
  const sfModel = model || process.env.SILICONFLOW_MODEL || 'Qwen/Qwen3-235B-A22B-Instruct-2507';
...
async function verifyCategoryMatch(productName, categorySlug, categoryNameAr) {
  ...
  const sfModel = process.env.SILICONFLOW_MODEL || 'Qwen/Qwen3-235B-A22B-Instruct-2507';
  try {
