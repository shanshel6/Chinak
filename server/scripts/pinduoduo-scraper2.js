...
async function enrichWithAI(title, description, price) {
    ...
    try {
        ...
        - "Pack" or "Package" often means "Bundle" or "Set", NOT "Backpack" (حقيبة ظهر). Only use "حقيبة ظهر" if it is literally a bag for wearing on the back.
...
const BROWSER_OVERRIDE = process.env.PDD_CHROME_PATH || process.env.PDD_EDGE_PATH || '';
const BROWSER_PATHS = [
...
function getExecutablePath() {
  if (BROWSER_OVERRIDE && fs.existsSync(BROWSER_OVERRIDE)) return BROWSER_OVERRIDE;
...
const applyMobileEmulation = async (page) => {
    if (!page) return;
    try { await page.setUserAgent(MOBILE_USER_AGENT); } catch (e) {}
    try { await page.setViewport({ width: 360, height: 800, isMobile: true, hasTouch: true }); } catch (e) {}
};
const waitForLoginCompletion = async (page, timeoutMs = 120000) => {
    ...
    while (Date.now() - start < timeoutMs) {
        const stillOnLogin = await safeEvaluate(page, () => {
            ...
            return url.includes('login') || s.includes('登录') || s.includes('短信登录') || s.includes('验证码');
        }).catch(() => false);
...
const applyRateLimitDelay = async () => {
    if (rateLimitBackoffMs <= 0) return;
    const now = Date.now();
    const waitMs = Math.max(0, rateLimitBackoffMs - (now - lastRateLimitAt));
    if (waitMs > 0) {
        console.log(`Rate limit cooldown ${Math.ceil(waitMs / 1000)}s...`);
        await delay(waitMs);
    }
    rateLimitBackoffMs = Math.max(15000, Math.floor(rateLimitBackoffMs * 0.8));
...
async function createBrowser() {
  ...
  const useGuest = String(process.env.PDD_GUEST || '1') === '1';
  const appDataRoot = process.env.LOCALAPPDATA || '';
  const isChrome = executablePath.toLowerCase().includes('chrome.exe');
  const defaultUserDataDir = appDataRoot
    ? path.join(appDataRoot, isChrome ? 'Google' : 'Microsoft', isChrome ? 'Chrome' : 'Edge', 'User Data')
    : (isChrome ? 'chrome_user_data' : 'edge_user_data');
  const userDataDir = useGuest ? undefined : (process.env.PDD_USER_DATA_DIR || defaultUserDataDir);
  const args = [
    '--no-sandbox',
  ...
  if (!useGuest && userDataDir) launchOptions.userDataDir = userDataDir;

  try {
    return await puppeteer.launch(launchOptions);
  } catch (e) {
    const msg = String(e?.message || '');
    if (!useGuest && msg.includes('already running')) {
        const requireProfile = String(process.env.PDD_REQUIRE_PROFILE || '0') === '1';
        if (requireProfile) {
            console.error('Chrome profile is in use. Close all Chrome windows and retry.');
            process.exit(1);
        }
        const fallbackDir = path.join(os.tmpdir(), `pdd_profile_${Date.now()}`);
        console.log(`Profile in use. Falling back to temporary profile: ${fallbackDir}`);
        return await puppeteer.launch({ ...launchOptions, userDataDir: fallbackDir });
    }
    throw e;
  }
}
...
async function run() {
    ...
    try {
        const skipCookies = String(process.env.PDD_SKIP_COOKIES || '0') === '1';
        if (!skipCookies) {
    ...
    const useMobileListing = String(process.env.PDD_MOBILE_LISTING || '0') === '1';
    const forceMobileAfterLogin = String(process.env.PDD_FORCE_MOBILE_AFTER_LOGIN || '0') === '1' || useMobileListing;
    const loginUrl = 'https://mobile.pinduoduo.com/login.html';
    console.log('Visiting Login URL first to ensure valid session...');
    try {
        await safeGoto(page, loginUrl, { waitUntil: 'domcontentloaded', timeout: PAGE_LOAD_TIMEOUT });
    ...
    console.log('================================================================');
    await delay(60000);
    if (forceMobileAfterLogin) {
        console.log('Switching to Mobile Viewport after login...');
        await applyMobileEmulation(page);
        await delay(2000);
    }
    ...
    try {
        ...
        if (searchKey) {
            cleanUrl = useMobileListing
                ? `https://mobile.pinduoduo.com/search_result.html?search_key=${encodeURIComponent(searchKey)}&search_type=goods`
                : `https://www.pinduoduo.com/search_result.html?search_key=${encodeURIComponent(searchKey)}&search_type=goods`;
        } else if (catId) {
            cleanUrl = useMobileListing
                ? `https://mobile.pinduoduo.com/catgoods.html?cat_id=${catId}&opt_id=${optId || catId}`
                : `https://www.pinduoduo.com/catgoods.html?cat_id=${catId}&opt_id=${optId || catId}`;
        } else if (optId) {
                cleanUrl = useMobileListing
                    ? `https://mobile.pinduoduo.com/catgoods.html?opt_id=${optId}`
                    : `https://www.pinduoduo.com/catgoods.html?opt_id=${optId}`;
        }
    ...
    for (let i = 0; i < 3; i++) {
        const isListingSoldOut = await safeEvaluate(page, () => {
            const text = document.body && document.body.innerText ? document.body.innerText : '';
            return String(text || '').includes('商品已售罄') || String(text || '').includes('推荐以下相似商品');
        }).catch(() => false);
        if (!isListingSoldOut) break;
        console.log('Sold-out banner detected on listing. Retrying...');
        await delay(4000 + Math.floor(Math.random() * 2000));
        await page.reload({ waitUntil: 'domcontentloaded', timeout: PAGE_LOAD_TIMEOUT }).catch(() => null);
        if (useMobileListing || forceMobileAfterLogin) {
            await applyMobileEmulation(page);
        }
        await waitForStableUrl(page, 1200, 12000);
    }
    ...
    const manualReady = String(process.env.PDD_MANUAL_READY || '0') === '1';
    if (manualReady) {
        await askQuestion('When the product list looks normal, press Enter to continue...');
    }
    ...
    */
    ...
    while (isRunning) {
        ...
        const loopDelayMs = Number(process.env.PDD_LOOP_DELAY_MS || 2500);
        if (loopDelayMs > 0) {
        ...
        const clickPoints = [
            { name: "Top-Left",  xPct: 0.25, yPct: 0.45 },
            { name: "Top-Right", xPct: 0.75, yPct: 0.45 },
            { name: "Bot-Right", xPct: 0.75, yPct: 0.75 },
            { name: "Bot-Left",  xPct: 0.25, yPct: 0.75 }
        ];
        ...
        for (const point of clickPoints) {
            ...
            const clickCoordinates = await page.evaluate((pt) => {
                 ...
                 const isHomeLink = (el) => {
                     ...
                     const href = a?.href || '';
                     return href === 'https://m.pinduoduo.com/' || href === 'https://m.pinduoduo.com';
                 };
                 const isHeader = (el) => {
                     if (!el) return false;
                     ...
                     return /header/i.test(cls);
                 };
                 const pick = (xPct, yPct) => {
                     const baseX = Math.floor(width * xPct);
                     const baseY = Math.floor(height * yPct);
                     const offsets = [0, 0.12, 0.24];
                     for (const off of offsets) {
                         const y = clamp(baseY + Math.floor(height * off), 1, Math.floor(height - 2));
                         const x = clamp(baseX, 1, Math.floor(width - 2));
                         const el = document.elementFromPoint(x, y);
                         if (!el) continue;
                         if (isHeader(el) || isHomeLink(el)) continue;
                         return { x, y };
                     }
                     return { x: clamp(baseX, 1, Math.floor(width - 2)), y: clamp(baseY, 1, Math.floor(height - 2)) };
                 };
                 const ptPicked = pick(pt.xPct, pt.yPct);
                 const randomX = Math.floor(Math.random() * 30) - 15;
                 const randomY = Math.floor(Math.random() * 30) - 15;
                 return {
                     x: clamp(ptPicked.x + randomX, 1, Math.floor(width - 2)),
                     y: clamp(ptPicked.y + randomY, 1, Math.floor(height - 2))
                 };
            }, point);
            ...
            try {
                // Show visual indicator (Blue Dot)
                try {
            ...
            for (let i = 0; i < 8; i++) {
                if ((Date.now() - lastProductAt) < 1500 && lastProductAt >= clickAt) break;
                try {
                    const u = page.url();
                    console.log(`[Nav] Post-click URL: ${u}`);
                    if (u && u !== currentUrl) break;
                } catch (e) {}
                await delay(250);
            }
            ...
            try {
                ...
                if (target && target.type() === 'page') {
                    ...
                    if (useMobileListing) {
                        await applyMobileEmulation(newPage);
                    }
                    ...
                    console.log(`[Nav] Same-tab URL check: ${nextUrl}`);
                    const isListingHome = nextUrl === 'https://m.pinduoduo.com/' || nextUrl === 'https://m.pinduoduo.com';
                    const isListingPage = nextUrl.includes('catgoods') || nextUrl.includes('search_result') || nextUrl.includes('search') || nextUrl.includes('opt');
                    if (nextUrl !== currentUrl && !isListingHome && !isListingPage) {
            ...
            try {
                ...
                const normalizedUrl = normalizeProductUrl(newPage.url());
                if (normalizedUrl && normalizedUrl !== newPage.url()) {
                    await safeGoto(newPage, normalizedUrl, { waitUntil: 'domcontentloaded', timeout: PAGE_LOAD_TIMEOUT });
                    await waitForStableUrl(newPage, 1200, 12000);
                }
                const curUrl = newPage.url();
                if (!isProductUrl(curUrl)) {
                    console.log(`Non-product URL detected. Returning to listing: ${curUrl}`);
                    if (!navigationHappened && newPage.close) await newPage.close();
                    else {
                        try { await newPage.goBack({ waitUntil: 'domcontentloaded', timeout: PAGE_LOAD_TIMEOUT }); } catch (e) {}
                    }
                    continue;
                }
                ...
                */
                    ...
                    try {
                     ...
                     const captureImages = () => {
                         ...
                         const sliderSelectors = [
                             '.QFNLpbqP img', // User Priority Selector
                             '.QFNLpbqP',     // User Priority Selector (Direct)
                             '.goods-slider img',
                             '.swiper-slide img',
                         ...
                         imgs.forEach(img => {
                             // Use a slightly lower width threshold for the specific user selector as it might be a small container
                             const minWidth = (img.closest('.QFNLpbqP')) ? 200 : 400;

                             if (img.src && img.src.startsWith('http') && img.naturalWidth > minWidth) {
                                 // Clean the URL
                     ...
                     // 1. User Priority Selector: .QFNLpbqP
                     let mainPriorityImg = document.querySelector('.QFNLpbqP img') || document.querySelector('.QFNLpbqP');
                     
                     // 2. Strict Slider Selector (if user selector not found)
                     if (!mainPriorityImg) {
                        mainPriorityImg = document.querySelector('.goods-slider img') || 
                     ...
                     // Validation: If strict image is a banner (wide), ignore it
                     if (mainPriorityImg) {
                        if (mainPriorityImg.naturalHeight > 0) {
                            const aspect = mainPriorityImg.naturalWidth / mainPriorityImg.naturalHeight;
                            if (aspect > 2.2 || aspect < 0.4) {
                                mainPriorityImg = null;
                            }
                        }
                     }

                     // 3. Fallback: Smart Area Search
                     if (!mainPriorityImg) {
                        ...
                        if (bestImg) mainPriorityImg = bestImg;
                     }
                                         
                     if (mainPriorityImg && mainPriorityImg.src && mainPriorityImg.src.startsWith('http')) {
                        const mainSrc = mainPriorityImg.src.split('?')[0];
                ...
                // --- ROBUST RETURN TO PRODUCT PAGE (Fix for 3x loop issue) ---
                // Ensure we are back on the product page before proceeding
                console.log(`[Reviews] Ensuring return to product page...`);
                let backAttemptsReview = 0;
                const maxBackAttemptsReview = 4;
                let onProductReview = false;

                while (backAttemptsReview < maxBackAttemptsReview) {
                    try {
                        const currentUrl = page.url();
                        const onReviews = currentUrl.includes('comment') || currentUrl.includes('reviews');
                        onProductReview = isProductUrl(currentUrl) && !onReviews;
                        const isCat = isCategoryUrl(currentUrl);
                        
                        if (onProductReview) {
                            console.log('[Reviews] Successfully confirmed on product page.');
                            break;
                        }
                        if (isCat) {
                             console.log('[Reviews] Landed on category page prematurely. Stopping back navigation.');
                             break;
                        }

                        if (onReviews) {
                             console.log(`[Reviews] Still on reviews (URL: ${currentUrl}). Going back (Attempt ${backAttemptsReview + 1})...`);
                             await page.goBack();
                             await humanDelay(2000, 3500);
                        } else {
                            // If not on reviews and not on product, maybe we drifted? 
                            // Assume we need to go back.
                             console.log(`[Reviews] Not on product/reviews (URL: ${currentUrl}). Going back (Attempt ${backAttemptsReview + 1})...`);
                             await page.goBack();
                             await humanDelay(2000, 3500);
                        }
                    } catch (e) {
                        console.log(`[Reviews] goBack() error: ${e.message}`);
                    }
                    backAttemptsReview++;
                }
            ...
            if (!navigationHappened) {
                ...
                let backAttemptsFinal = 0;
                const maxBackAttemptsFinal = 4;

                while (backAttemptsFinal < maxBackAttemptsFinal && !isCategoryPage) {
                    console.log(`⚠️ Not back on category page yet. Going back again (${backAttemptsFinal + 1}/${maxBackAttemptsFinal})...`);
                    try {
                        await page.goBack({ waitUntil: 'domcontentloaded', timeout: 15000 });
                    } catch (e) {
                        console.log('goBack failed or timed out:', e.message);
                    ...
                    if (isCategoryPage) break;
                    
                    backAttemptsFinal++;
                }

                if (!isCategoryPage) {
                    console.log('⚠️ Could not reach category page after back attempts. Forcing navigation to Category URL...');
                    try {
                        await page.goto(CATEGORY_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
                        await humanDelay(3000, 5000);
                        // Reset scroll if we force reload
                        accumulatedScrollY = 0; 
                    } catch (e) {
                        console.log('Force navigation failed:', e.message);
                    }
                }
            }
